...
export default function SplashCursor({
  ...
  CURL = 3,
  // larger defaults so the effect appears bigger on screen
  SPLAT_RADIUS = 0.38,
  SPLAT_FORCE = 14000,
  SHADING = true,
  ...
  TRANSPARENT = true,
  className = "",
}: SplashCursorProps & { className?: string }) {
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      const halfFloatTexType = isWebGL2
        ...
        : (halfFloat && (halfFloat.HALF_FLOAT_OES ?? 0)) || 0;

      // formats will be determined based on support
  ...
  return (
    <div
      className={`${className} fixed top-0 left-0 z-0 pointer-events-none w-full h-full`}
    >
      <canvas
        ref={canvasRef}
        id='fluid'
